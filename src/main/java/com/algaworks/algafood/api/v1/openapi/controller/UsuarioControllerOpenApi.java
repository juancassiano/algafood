...
public interface UsuarioControllerOpenApi {
    ...
    @Operation(summary = "Busca um usuário por ID", responses = {
            ...
            @ApiResponse(responseCode = "404", description = "ID do usuário não encontrado", content = @Content(schema = @Schema(ref = "Problema")))

    })
    UsuarioModel buscar( @Parameter(description = "ID do usuário", example = "1", required = true) Long usuarioId);

    @Operation(summary = "Adiciona um usuário", responses = {
            @ApiResponse(responseCode = "201", description = "Usuário cadastrado", content = @Content(schema = @Schema(ref = "Problema")))

    })
    UsuarioModel adicionar(@RequestBody( description = "Representação de um novo usuário", required = true)
                            UsuarioComSenhaInput usuarioComSenhaInput);

    @Operation(summary = "Atualiza um usuário por ID", responses = {
            @ApiResponse(responseCode = "200", description = "Usuário atualizado", content = @Content(schema = @Schema(ref = "Problema"))),
            @ApiResponse(responseCode = "404", description = "ID do usuário não encontrado", content = @Content(schema = @Schema(ref = "Problema")))

    })
    UsuarioModel atualizar(@Parameter(description  = "ID do usuário", example = "1", required = true)
                           Long usuarioId,
